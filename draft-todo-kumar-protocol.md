...
Additionally, further research can address the general concern with having single points of failure and how that relates to hardware in addition to the network itself. This can encompass research into how the different layers of the internet and software/applications and hardware relate to each other  in relation to the issue of single points of failure (for example, by elucidating the kinds of harms that could arise from the exacerbation of single points of failure from protocol development and implementation).
