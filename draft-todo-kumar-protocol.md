...
At IETF 117, there were two presentations of particular interest to this draft: that of Nick Merrill and of Baltra. 
...
# 4. Recommendations 
From this framework, we aim to promote developers and researchers working in the IRTF/IETF space to consider how their work could have a negative impact on end-users, with the focus here being on centralisation and the end-to-end principle. Developing metrics for quantitatively measuring this (the ‘how’) may be beyond the scope of this document, but qualitative indicators for the risk to the end-user could be an option to explore further. The IRTF has a responsibility to consider how the protocols developed under its auspices impact end-users: ensuring that protocols are grounded in the principles of promoting competition, open standards, and equitable access to the internet can serve as a starting point to reversing centralisation trends. 

If, as RFC 8890 states “Inappropriate concentration of power on the Internet has become a concerning phenomenon -- one that protocol design might have some influence upon” [Nottingham 2], then we posit that the IETF could explore how protocol design might have an influence on this concentration by centring the end user. Specifically, the use of a human rights impact assessment [Grover] can ensure that the end-user perspective is integrated into the design, development and deployment of protocols. 

Building on the set of human rights protocol considerations developed by the Human Rights Protocol Considerations (HRPC) Research Group [Grover], we have identified a set of guiding questions of relevance to centralisation or fragmentation, focusing on how these elements are relevant to the principal connections of concern: 1) the advancement of single points of failure 2) impact on user choice”. This proposed framework aims to serve as a starting point for consideration of these two key elements

4.2 Suggested Areas for Further Research
To address the core trends leading consolidation, however, requires examination of the broader internet ecosystem. Equality in access to the internet - particularly in the Global South - grounded in a competitive market of Internet Service Providers (ISPs) is integral to reversing consolidation trends. These will require broader policy and regulatory efforts, particularly to provide economic incentives to diversify the market, and the IETF should use its convening role to research and propose potential solutions.  For example, the IETF could host more discussions on protocol design and impact to augment or curtail concentration of power to support the work of decisionmakers.

# 5. Use Cases 

For the use cases below, we could ask “do standards preserve end-user choice rights and increase/reinforce, decrease choke points or are they neutral?” In exploring this question, and because of the trends above in relation to the end-to-end principle, it will also be instructive to address “how do current trends in terms of the rise in third parties and their corresponding impact on consolidation impact the end-to-end principle?” As protocol design is about evolution: can any claims be made that at any point a protocol design further consolidates internet infrastructure OR that it undermines end-user agency? A user-centred analysis via the following use cases could therefore employ a three-pronged approach on 1) end-user choice, control and end user rights; 2) on choke points and 3) the end-to end principle.
